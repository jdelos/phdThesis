...
The fast switching limit (FSL) equivalent output resistance $r_{fsl}$ accounts for losses produced in the resistive circuit elements, being these the \emph{on}-resistance of the switches and the Equivalent Series Resistance (ESR) of the capacitors $r_{esr,c}$. As outlined in~\cite{Seeman:EECS-2009-78}, the charge flow vectors through the parasitic resistive elements $\mathbf{ar}$ can be derived from the charge flow vectors $\mathbf{a}$, then $r_{fsl}$ is computed as
\begin{equation}
